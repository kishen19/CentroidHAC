...
//file order: {PointsFile, IndexFile, SaveIndexFile, ErrorsFile}
...
void Centroid_main(parlay::sequence<char*> files, bool test, bool exact, 
  ...
  if (exact){
    ...
    CentroidHAC_errors<PointRange, indexType, nn_exact<PointRange, indexType>>(Points, NN, eps, files[2], files[3]);
  } else{
    auto NN = nn_knn<PointRange, indexType>(files[1], R, L, alpha, pass, Points.size());
    CentroidHAC_errors<PointRange, indexType, nn_knn<PointRange, indexType>>(Points, NN, eps, files[2], files[3]);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  char* ErrorsFile = P.getOptionValue("-errors_outfile");
  ...
  parlay::sequence<char*> files = {PointsFile, IndexFile, SaveIndexFile, ErrorsFile};

  if(tp == "float"){
    Centroid_main<PointRange<float, Euclidean_Point<float>>, uint>(files, test, exact, eps, R, L, alpha, pass, num_clusters, cluster_size, MST_deg, delta);
  } else if(tp == "uint8"){
    Centroid_main<PointRange<uint8_t, Euclidean_Point<uint8_t>>, uint>(files, test, exact, eps, R, L, alpha, pass, num_clusters, cluster_size, MST_deg, delta);
  } else if(tp == "int8"){
    Centroid_main<PointRange<int8_t, Euclidean_Point<int8_t>>, uint>(files, test, exact, eps, R, L, alpha, pass, num_clusters, cluster_size, MST_deg, delta);
  }
  return 0;
}
