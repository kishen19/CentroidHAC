...
if [ $build_index -eq 1 ]
then
    bazel run //src:centroid_main -- -R $R -L $L -alpha $a -data_type $dt -base_path ${base_path}${dataset}.txt -test -graph_outfile ${base_path}${dataset}_32_64 -dendrogram_outfile ${base_path}${dataset}_dend_${eps}.txt -eps ${eps}
else
